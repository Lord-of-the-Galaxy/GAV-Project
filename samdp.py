...
def kmeans_st(coords, values, n_clusters, window_size=4, window_weight_falloff=0.75):
    ...
    for i in range(window_size*2+1):
        vecs[:, 2*i] = (window_weight_falloff**(abs(i-window_size))) * \
            np.roll(coords, i-window_size, axis=0)[:, 0]
        vecs[:, 2*i + 1] = (window_weight_falloff**(abs(i-window_size))) * \
            np.roll(coords, i-window_size, axis=0)[:, 1]
    vecs[:, window_size*4 + 2] = values
    kmeans = MiniBatchKMeans(n_clusters=n_clusters,
                             random_state=int(time()), n_init=3).fit(vecs)
...
def samdp(activation_data, values, rewards, n_clusters=20, window_size=4, min_skill_length=4, window_weight_falloff=0.75, threshold=0.01, gamma=0.99):
    coords, cluster_values, labels, inertia = kmeans_st(
        activation_data, values, n_clusters, window_size, window_weight_falloff)
    T = compute_transition_matrix(n_clusters, labels, min_skill_length)
    P = compute_prob_matrix(T, threshold)
    samdp_rewards, skill_lengths = calc_samdp_rewards(
        n_clusters, labels, rewards, min_skill_length, gamma)
...
def samdp_search(activation_data, values, rewards, min_clusters, max_clusters, min_window_size, max_window_size, min_skill_length=4, window_weight_falloff=0.75, threshold=0.01, gamma=0.99):
    vmse = np.zeros((max_clusters - min_clusters + 1) *
                    (max_window_size - min_window_size + 1))
    inertia = np.zeros((max_clusters - min_clusters + 1) *
                       (max_window_size - min_window_size + 1))
    entropy = np.zeros((max_clusters - min_clusters + 1) *
                       (max_window_size - min_window_size + 1))
    ...
    for i in range(min_clusters, max_clusters+1):
        for j in range(min_window_size, max_window_size+1):
            ...
            coords_, v_samdp_, labels_, P_, vmse[(i-min_clusters)*(max_window_size - min_window_size + 1) + j-min_window_size], inertia[(i-min_clusters)*(max_window_size - min_window_size + 1) + j-min_window_size], entropy[(
                i-min_clusters)*(max_window_size - min_window_size + 1) + j-min_window_size] = samdp(activation_data, values, rewards, i, j, min_skill_length, window_weight_falloff, threshold, gamma)
    ...
    for p in range(vmse.shape[0]):
        ...
        candidates = set(vmse_min).intersection(
            set(inertia_min)).intersection(set(entropy_min))
        if candidates:
            print("Found candidate at p =", p)
            n = candidates.pop()
            n_clusters, window_size = n//(max_window_size - min_window_size + 1) + min_clusters, n % (
                max_window_size - min_window_size + 1) + min_window_size
...
def main():
    ...
    tsne = TSNE(n_components=2, perplexity=120, n_neighbors=600,
                verbose=1, random_state=1534, method='fft')
    print("Running tSNE")
    ...
    nc, coords, v_samdp, labels, P, vmse, inertia, entropy = samdp_search(
        data_t, values, rewards, 15, 25, 2, 5)
        ...
        for j in range(nc):
            if P[i, j] > 0.05:
                ...
                plt.plot([coords[i, 0], coords[j, 0]], [
                         coords[i, 1], coords[j, 1]], color='black', linewidth=P[i, j]*5, alpha=0.5)
